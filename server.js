...
function calculatePnl(history, periodHours = DEFAULT_PNL_PERIOD_HOURS, initialCapital = DEFAULT_PNL_CAPITAL_USD) {
  ...
  const records = history.filter(item => Date.parse(item.detectedAt) >= cutoff && item.opportunitySnapshot).sort((a, b) => Date.parse(a.detectedAt) - Date.parse(b.detectedAt)); let availableCapital = startingCapital; const attempts = []; const skipped = [];
  for (const record of records) { if (availableCapital <= 0) { skipped.push(record); continue; } const simulation = simulateOpportunity(record.opportunitySnapshot, availableCapital); const fundedCapital = simulation.capitalUsd; const expectedDelta = Math.max(-availableCapital, Math.min(availableCapital, simulation.netUsd)); availableCapital = round(Math.max(0, availableCapital + expectedDelta), 2); attempts.push({ detectedAt: record.detectedAt, pair: record.pair, simulation, fundedCapitalUsd: fundedCapital, expectedDeltaUsd: round(expectedDelta) }); }
  const sums = attempts.reduce((acc, item) => { const sim = item.simulation; acc.expected += item.expectedDeltaUsd; acc.persisted += Math.max(-item.fundedCapitalUsd, Math.min(item.fundedCapitalUsd, sim.persistedNetUsd)) * sim.persistenceProbability; acc.reverted += Math.max(-item.fundedCapitalUsd, Math.min(item.fundedCapitalUsd, sim.revertedNetUsd)) * sim.reversionProbability; acc.best += Math.max(-item.fundedCapitalUsd, Math.min(item.fundedCapitalUsd, sim.persistedNetUsd)); acc.worst += Math.max(-item.fundedCapitalUsd, Math.min(item.fundedCapitalUsd, sim.revertedNetUsd)); acc.reversionProbability += sim.reversionProbability; return acc; }, { expected: 0, persisted: 0, reverted: 0, best: 0, worst: 0, reversionProbability: 0 });
  const expected = Math.max(-startingCapital, Math.min(availableCapital - startingCapital, sums.expected)); return { periodHours: hours, periodLabel: hours < 48 ? `${hours} hours` : `${round(hours / 24, hours % 24 ? 1 : 0)} days`, startingCapitalUsd: round(startingCapital), endingCapitalUsd: round(startingCapital + expected), pnlUsd: round(expected), pnlPct: round(expected / startingCapital * 100, 3), attempts: records.length, fundedAttempts: attempts.length, skippedAttempts: skipped.length, persistedContributionUsd: round(sums.persisted), revertedLossUsd: round(sums.reverted), bestCaseUsd: round(sums.best), worstCaseUsd: round(sums.worst), averageReversionProbability: attempts.length ? round(sums.reversionProbability / attempts.length * 100, 1) : 0, coverage: records.length ? { from: records[0].detectedAt, to: records[records.length - 1].detectedAt } : null };
}
async function scan() {
